...
import { type Rule, Rule_ID } from '../types/Rule'

function isValidV(value: unknown): value is V {
  return typeof value === 'number' && 1 <= value && value <= 9
}

function isValidRule(value: unknown): value is Rule {
  return typeof value === 'object' && value !== null && 'type' in value && Rule_ID.includes((value as any).type)
}
